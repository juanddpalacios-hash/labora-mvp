...
### Regla 25. ROLE_INTENT_GATE: Analista Comercial requiere cerca-personas
`analista-comercial-junior` requiere `cerca-personas` en interests (absent_penalty: 18).
...
## Estado actual (2026-04-13) — post sprint auditoría cuestionario bloques 1 y 2

### Completado — Sprint auditoría cuestionario bloques 1 y 2 (2026-04-13, commit e309203)

**Objetivo:** Revisar y rediseñar los inputs del cuestionario explore para maximizar la calidad de señal sin añadir complejidad. Cada bloque auditado con tests de validación antes y después.

---

### Bloque 1: BEHAVIORAL_INTERESTS rediseñado

**Problema resuelto:**
Las 8 opciones anteriores tenían dominios mezclados por opción (ej: `cerca-personas` apuntaba a comercial + personas + marketing). Esto producía señal ambigua: el usuario podía estar eligiendo por un dominio pero el motor acumulaba traits de otro.

**Principio aplicado:**
> Cada opción = un dominio único. El usuario elige "tipo de trabajo", no "área".

**Cambios en `public/app.js`:**
- `aprender-profundo` eliminado — señal de aprendizaje universal, no discrimina dominio
- Máx selecciones: 3 → 2
- Todos los IDs mantenidos para compatibilidad con `ROLE_INTENT_GATE` y `DOMAIN_SIGNAL_MAP`
- Semántica ajustada: `mejorar-organizacion` → personas/RRHH; `cerca-personas` → comercial puro; `crear-impacto` → marketing puro; `coordinar-avanzar` → proyectos

**BEHAVIORAL_INTERESTS final (7 opciones):**
| ID | Label | Dominio | Traits |
|---|---|---|---|
| `entender-datos` | "Trabajar con datos para encontrar patrones y convertirlos en decisiones útiles" | analitica | analisis+2, aprendizaje+1 |
| `numeros-negocio` | "Trabajar con los números del negocio — ingresos, costos y resultados — para entender si está funcionando bien y dónde se puede mejorar" | finanzas/cdg | analisis+2 |
| `procesos-ordenados` | "Mejorar cómo funcionan las cosas en el día a día, detectando problemas y haciendo que todo sea más eficiente" | operaciones | ejecucion+2, coordinacion+1 |
| `coordinar-avanzar` | "Sacar adelante proyectos con un objetivo claro y una fecha determinada, organizando tareas y equipos para lograrlo" | proyectos | coordinacion+2, aprendizaje+1 |
| `mejorar-organizacion` | "Atraer, seleccionar y ayudar a integrar personas en una organización" | personas | social+2, coordinacion+1 |
| `cerca-personas` | "Trabajar directamente con clientes, entendiendo lo que necesitan y ayudando a concretar ventas o acuerdos" | comercial | contacto_cliente+2, social+1 |
| `crear-impacto` | "Crear campañas o acciones para atraer clientes y aumentar el interés por un producto o servicio" | marketing | analisis+1, coordinacion+1, social+1 |

**Cambios en `server/services/roleMatcher.js`:**
- `INTEREST_TO_TRAITS`: actualizado para 7 IDs (removido `aprender-profundo`)
- `DOMAIN_SIGNAL_MAP`: ajustado para nueva semántica de IDs
  - `analytics`: solo `entender-datos` (sin `aprender-profundo`)
  - `marketing`: solo `crear-impacto` (sin `cerca-personas`)
  - `people_org`: `mejorar-organizacion`, `coordinar-avanzar`, `cerca-personas`
  - `communications`: sin señales habilitadoras (antes tenía `cerca-personas`)
- `DOMAIN_FIT_MAP`: `projects` agregado al listado `nearby` de IC
- `ROLE_INTENT_GATE` CdG: `mejorar-organizacion` removido de `required_signals.interests` (ese ID ahora = personas, no cdg)

**Cambios en `data/junior_roles.json`:**
- Asistente de Proyectos Junior: `domain: "people_org"` → `"projects"`
- Project Manager Junior: `domain: "people_org"` → `"projects"`

**Cambios en `public/app.js`:**
- `buildConfirmExplanation()`: IDs de señales corregidos para reflejar nueva semántica
  - `hasPeopleInterest`: antes usaba `cerca-personas` y `coordinar-avanzar` (ahora son comercial y proyectos) → ahora usa `mejorar-organizacion`
  - `hasOpsInterest`: antes usaba `mejorar-organizacion` (ahora es personas) → ahora usa `procesos-ordenados`

**Comportamiento verificado post-rediseño:**
- Analytics (`entender-datos` + `analizar-datos`): Datos/BI dominan, Riesgo pierde terreno ✅
- Finance (`numeros-negocio`): Inversiones/CdG/Riesgo top ✅
- Operations (`procesos-ordenados`): Ops roles top, Contable/Tesorería con natural_weak ✅
- Projects (`coordinar-avanzar`): PM Junior y Asistente Proyectos reciben nearby_strong +4 ✅
- People (`mejorar-organizacion`): RRHH/Reclutamiento/DO top ✅
- Commercial (`cerca-personas`): Ejecutivo Comercial/Ventas dominan ✅
- Marketing (`crear-impacto`): Marketing Digital/Performance top ✅

---

### Bloque 2: EXPLORE_TASKS rediseñado

**Problema resuelto:**
Los labels anteriores no capturaban con suficiente precisión el estilo de trabajo. En particular:
- `resolver-problemas` tenía `trabajar-personas` en sus traits, mezclando señal social con señal de exploración/iteración
- `trabajar-personas` tenía `contacto_cliente` en sus traits, mezclando coordinación interna con perfil comercial

**Principio aplicado:**
> Bloque 2 modula el perfil dentro del dominio elegido en bloque 1. "Qué haces" × "Cómo lo haces" = perfil especializado.

**TASK_TO_TRAITS actualizado:**
```js
const TASK_TO_TRAITS = {
  "analizar-datos":     { analisis: 2, aprendizaje: 1 },
  "resolver-problemas": { ejecucion: 1, aprendizaje: 2 },   // antes: analisis+1, ejecucion+1
  "trabajar-personas":  { coordinacion: 2, social: 1 },      // antes: social+2, coordinacion+1, contacto_cliente+1
  "organizar-procesos": { ejecucion: 2, coordinacion: 1 },
  "crear-estrategias":  { analisis: 1, coordinacion: 1, aprendizaje: 1 }
};
```

**EXPLORE_TASKS labels finales:**
| Value | Label |
|---|---|
| `analizar-datos` | "Me meto a fondo en un tema, organizo lo que encuentro y solo entonces propongo algo" |
| `resolver-problemas` | "Empiezo con una versión rápida de una idea, la pruebo y la ajusto con lo que voy aprendiendo" |
| `trabajar-personas` | "Me muevo entre personas y equipos para coordinar lo que hace falta y que el trabajo siga avanzando" |
| `organizar-procesos` | "Ordeno el trabajo: defino qué hay que hacer, en qué orden va cada cosa y quién se encarga de qué" |
| `crear-estrategias` | "Veo lo que hay que hacer, identifico qué es más importante y decido por dónde empezar" |

**Decisión de diseño (registrada):**
- `organizar-procesos` mantuvo `ejecucion+2` (no se bajó a +1). La reducción no cerraba el gap de RRHH en marketing+organizar-procesos, pero sí debilitaba ops+organizar. La causa raíz es acumulación estructural de `coordinacion` desde 3 fuentes — contenida por domain_fit gap (+5 vs 0).

### Regla 21. BEHAVIORAL_INTERESTS: monodominio
Cada opción de BEHAVIORAL_INTERESTS debe mapear a un único dominio o par coherente de dominios. No mezclar áreas que compiten (ej: comercial + personas en la misma opción). La discriminación entre dominios depende de que el usuario pueda elegir uno sin activar señales del otro.

### Regla 22. TASK_TO_TRAITS: estilo de trabajo ≠ perfil social
`trabajar-personas` en EXPLORE_TASKS describe coordinación interna, no contacto con clientes externos.
`contacto_cliente` solo debe aparecer en tasks con semántica explícitamente comercial.
`cerca-personas` en BEHAVIORAL_INTERESTS es la señal comercial — no duplicar vía tasks.
...
## Estado actual (2026-04-13) — post sprint rediseño bloque 3

### Completado — Sprint rediseño bloque 3 EXPLORE_AVOID (2026-04-13)

**Problema resuelto:**
Las 5 opciones anteriores mezclaban condiciones emocionales (presión, repetitivo), conceptos de distinto nivel (dominio, estilo, cultura) y tenían overlap semántico entre sí. `trabajo-repetitivo` y `ambientes-competitivos` eran cuasi-universales y no discriminaban. `ventas-metas` y `atencion-clientes` se solapaban en roles comerciales.

**Principio aplicado:**
> Cada opción de bloque 3 representa un tipo de trabajo / cluster que el usuario quiere evitar. Actúa como penalizador suave por dominio — no genera traits, no es hard filter, no requiere lógica de contradicción.

**Nuevas opciones EXPLORE_AVOID:**

| ID | Label | Cluster penalizado | Penalty |
|---|---|---|---|
| `evitar-ventas` | "Trabajos donde hay que vender directamente o cumplir metas comerciales" | commercial | −18 (cc≥3) / −10 (cc≥2) |
| `evitar-analitica` | "Trabajos donde el foco principal es analizar datos para encontrar patrones o tendencias" | analytics | −15 |
| `evitar-finanzas` | "Trabajos donde hay que revisar números del negocio, presupuestos o indicadores financieros" | finance | −15 |
| `evitar-coordinacion` | "Trabajos donde el rol principal es coordinar personas, hacer seguimiento de equipos o llevar proyectos" | projects + people_org | −12 |
| `evitar-procesos` | "Trabajos estructurados donde la mayor parte del tiempo se ejecutan tareas definidas dentro de procesos establecidos" | operations | −12 |

**Cambios en `server/services/roleMatcher.js`:**
- `AVOID_PENALTY_RULES` reemplazado: 10 reglas por traits → 6 reglas por dominio (IDs nuevos)
- `computeAvoidPenaltyAdditive`: condición actualizada a `rule.condition(rTraits, role)` — ahora recibe el objeto role completo para chequeos de `role.domain`

**Cambios en `public/app.js`:**
- `EXPLORE_AVOID`: 5 opciones nuevas con nuevos IDs y labels
- `AVOID_NATURAL`: actualizado para los 5 nuevos IDs
- `buildExploreResultsHook`: señales avoid actualizadas (`avoidsVentas`, `avoidsAnalitica`, `avoidsFinanzas`, `avoidsCoordinacion`, `avoidsProcesos`); eliminados `avoidsTerrain` y `avoidsCompete` (sin equivalente en nuevo diseño)
- `buildConfirmExplanation`: ídem. Copy de ramas `analitico()`, `relacional()`, `operativo()`, `estrategico()` actualizado para usar nuevas señales

**Arquitectura inalterada:** `DOMAIN_SIGNAL_MAP`, `INTEREST_TO_TRAITS`, `TASK_TO_TRAITS`, `ROLE_INTENT_GATE`, `buildUserTraitVector`, `domain_fit_modifier`, guided mode — sin cambios.

**Comportamiento verificado (2026-04-13):**
- Analytics profile + evitar-ventas: Datos/BI top, comercial ausente ✓
- entender-datos + evitar-analitica (contradicción): analytics penalizado −15, finance/marketing toman el top — calibración por magnitud, sin lógica de contradicción ✓
- cerca-personas + evitar-ventas: Ejecutivo Comercial/Ventas fuera del top ✓
- evitar-finanzas + evitar-procesos: doble penalty en Asistente Contable (−27), sale del top ✓

### Regla 23. AVOID_PENALTY_RULES: condición recibe (traits, role)
Las condiciones de `AVOID_PENALTY_RULES` reciben `(t, r)` donde `r` es el objeto role completo.
No usar solo `(t)` — las reglas de bloque 3 necesitan `r.domain` para discriminar correctamente por cluster.
Si se agrega una nueva regla trait-based (sin chequeo de dominio), puede ignorar `r`, pero siempre declarar el segundo parámetro.

### Regla 24. EXPLORE_AVOID: IDs actuales (post 2026-04-13)
Los únicos IDs válidos en `avoid_preferences` son: `evitar-ventas`, `evitar-analitica`, `evitar-finanzas`, `evitar-coordinacion`, `evitar-procesos`.
Los IDs anteriores (`ventas-metas`, `atencion-clientes`, `trabajo-repetitivo`, `trabajo-terreno`, `ambientes-competitivos`) ya no existen en el sistema.

---

## Estado actual (2026-04-14) — post sprint optimización motor (MOTOR CERRADO PARA MVP)

### Completado — Sprint optimización motor (2026-04-14)

**Objetivo:** Mejorar calidad del top del ranking sin agregar nueva UI. Tres líneas de trabajo: gate para Analista Comercial, widening de domain_fit, ajuste de traits finance.

---

### Fix 1: ROLE_INTENT_GATE — Analista Comercial Junior

**Problema:** Analista Comercial aparecía en top-3 para perfiles sin señal comercial (ops/proyectos) por traits mixtos (analisis+social+ejecucion).

**Solución:** Nueva entrada en `ROLE_INTENT_GATE`:
```js
"analista-comercial-junior": {
  required_signals: { interests: ["cerca-personas"], tasks: [] },
  absent_penalty: 18
}
```
Sin `cerca-personas` → score cae ~18 pts, sale del top-3. Con `cerca-personas` → gate=0, sin penalización.

---

### Fix 2: Domain fit widening

**Problema:** Gap entre roles con señal directa y sin señal era de 3–4 pts — insuficiente para separación visual clara.

**Cambio:** `DOMAIN_MODIFIERS` en `roleMatcher.js`:
- `natural` (señal presente): +5 → **+8**
- `natural_weak` (sin señal): +2 (sin cambio)
- `nearby_strong` (señal presente): +4 → **+6**

**Resultado verificado (7 tests):**
- Gap señal→sin señal: 3 pts → 6+ pts en todos los dominios
- Top-2 con señal directa (★★) claramente separados de #3 sin señal
- Diversidad top-5 intacta — Business Analyst y Venture Analyst siguen apareciendo en slots 4–5

---

### Fix 3: Ajuste de traits finance

**Problema:** CdG y Presupuestos tenían vector idéntico (dist=0). Financiero/CdG/Riesgo a distancia 1.41.

**Cambios en `data/junior_roles.json`:**
| Rol | Dimensión | De → A | Justificación |
|---|---|---|---|
| CdG | `ejecucion` | 2 → 1 | Business partner: menos ejecución individual |
| CdG | `coordinacion` | 1 → 2 | El rol más cross-funcional de finance — coordina con todas las áreas |
| Presupuestos | `coordinacion` | 1 → 0 | Modelado de presupuesto es trabajo de cabeza baja, no cross-funcional |
| Riesgo | `presion` | 1 → 2 | Deadlines regulatorios duros, stress testing bajo plazos no negociables |

**Resultado:** CdG/Presupuestos: dist 0→2.24. Financiero/CdG: dist 1.41→2.45. Todos los pares finance ahora dist ≥1.0.

**Límite confirmado por experimento:** el empate intra-finance para perfiles genéricos (solo `numeros-negocio`) persiste porque el espacio de traits es estructuralmente saturado para roles analíticos de finance. El swap CdG eje/coo es algebraicamente neutro para user.coo=1 (punto de cruce de la fórmula). No resoluble con recalibración de traits.

---

### Decisión de cierre del motor

**Motor cerrado para MVP** — no implementar affinity_signals ni nuevos mecanismos de scoring.

**Justificación:** el motor cumple el objetivo de MVP (separación de dominios, avoids funcionales, no garbage crítico, ranking coherente). El retorno marginal de seguir optimizando es decreciente frente al valor de validar el producto con usuarios reales.

**Límites aceptados conscientemente:**
- Empate intra-finance para perfiles genéricos (Financiero/CdG/Inversiones ~41)
- Sin detección de convergencia multi-señal (suma lineal de traits no captura intención específica)
- Proxy temporal `numeros-negocio` para gate de Compliance

**Solución futura documentada (no implementar en MVP):**
`affinity_signals` por rol: campo en `junior_roles.json` con señales de convergencia por categoría (interests/tasks/motivations). Boost +7 cuando ≥2 fuentes independientes convergen. Diseño piloto para 3 roles: Analista de Datos, CdG, Ejecutivo Comercial.

---

### Regla 25. ROLE_INTENT_GATE: Analista Comercial requiere cerca-personas
`analista-comercial-junior` requiere `cerca-personas` en interests (absent_penalty: 18).
Traits mixtos (analisis+social+ejecucion) producen falsos positivos en perfiles ops/proyectos sin señal comercial.

### Regla 26. domain_fit: valores actuales post-widening (2026-04-14)
`DOMAIN_MODIFIERS`: natural=8, natural_weak=2, nearby_strong=6, nearby=0, distant=−4.
No revertir a valores anteriores (+5/+4) — el widening fue validado con 7 tests y es parte del motor base.

### Regla 27. Traits finance: identidades post-ajuste
- **CdG (3,1,2):** coordinacion:2 define su identidad de business partner cross-funcional
- **Presupuestos (3,2,0):** coordinacion:0 define trabajo de modelado sin coordinación cross-funcional
- **Riesgo (3,1,0,0,0,2,3):** presion:2 refleja deadlines regulatorios duros
- No reverter estos valores — son semánticamente correctos y rompen el par idéntico CdG/Presupuestos

### Prioridades post-cierre motor (Bloque 4)
30. **ROLE_PRACTICE_CONTENT para 44 roles** — base de toda la capa de acción del Bloque 4
31. **Jerarquía visual strong vs stretch** — top-1/2 como camino principal
